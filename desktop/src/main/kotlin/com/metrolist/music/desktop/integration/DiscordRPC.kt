    ...
    private fun connect(): Boolean {
        ...
        for (i in 0..9) {
            try {
                ...
                // Read response (should be READY event)
    ...
    private fun setPresence(song: SongInfo) {
        scope.launch {
            try {
                ...
                val thumbnailUrl = song.thumbnailUrl?.replace("w60-h60", "w512-h512")?.let { escapeJson(it) }
                ...
                // Read response but don't block on it
                try { readFrame() } catch (_: Exception) {}
            } catch (e: Exception) {
