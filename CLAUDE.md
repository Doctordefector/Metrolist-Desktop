...
- **Version must be updated in TWO places** when releasing:
  1. `desktop/build.gradle.kts` → `packageVersion = "X.Y.Z"` (MSI/EXE installer version)
  2. `desktop/.../update/AutoUpdater.kt` → `CURRENT_VERSION = "X.Y.Z"` (in-app display + update checker)
- Both MUST match — `packageVersion` controls the installer filename, `CURRENT_VERSION` is shown in Settings and used for update comparison
- Build commands:
  - `./gradlew :desktop:packageExe` → `desktop/build/compose/binaries/main/exe/Metrolist-X.Y.Z.exe`
  - `./gradlew :desktop:packageMsi` → `desktop/build/compose/binaries/main/msi/Metrolist-X.Y.Z.msi`
  - `./gradlew :desktop:createDistributable` → `desktop/build/compose/binaries/main/app/Metrolist/` (portable folder)

## Distribution Strategy
- **Every release MUST include all 3 artifacts:**
  1. `Metrolist-X.Y.Z.exe` — NSIS installer (recommended for most users)
  2. `Metrolist-X.Y.Z.msi` — Windows Installer (for managed/enterprise deployments)
  3. `Metrolist-X.Y.Z-portable.zip` — Portable version (no install, just extract and run)
- Portable ZIP created by zipping `desktop/build/compose/binaries/main/app/Metrolist/` after `createDistributable`
...
- Upload all 3 artifacts (EXE, MSI, portable ZIP) to each GitHub release

## Priority Work Items
1. **Podcast support**
2. **Music recognition**
3. **Library search/filter** — search within local library (not just YouTube)
4. **Sorting** — sort library songs/albums/artists by name, date, play count
